...
def register_team(message, color: str):
    text = f'*Team {color}*: ' + message.text
    bot.send_message(message.chat.id, text, parse_mode='Markdown')
    if len(message.text.split(' ')) != 5:
        bot.send_message(message.chat.id, '*Wrong number of players!*', parse_mode='Markdown')
        return
    if color == 'white':
...
def start_match(message):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    white_buttons = [types.KeyboardButton(f'{white_square}{white_player}')
                     for white_player in (current_game.white_team + ['own_black'])]
    black_buttons = [types.KeyboardButton(f'{black_square}{black_player}')
...
def get_time_message() -> str:
    ...
    return '*Current Time:* `' + str(hours) + 'h' + str(minutes) + 'm' + str(seconds) + 's`'
...
def result(message):
    ...
    for (scorer, timestamp) in zip(current_game.scorers, current_game.time_stamps):
        if scorer in current_game.white_team or scorer == 'own_black':
            score_white += 1
    text += '\n*Current Score:\nWhite*: ' + str(score_white) + '\n*Black*: ' + str(
        len(current_game.scorers) - score_white)
    bot.send_message(message.chat.id, text, parse_mode='Markdown')
...
def scoreboard(message):
    ...
    for (scorer, timestamp) in zip(current_game.scorers, current_game.time_stamps):
        ...
        if scorer in current_game.white_team or scorer == 'own_black':
            ...
            score_black += 1
        text += square + f' *{scorer}* ' + f' {timestamp}\' ({score_white}-{score_black})\n'
    bot.send_message(message.chat.id, text, parse_mode='Markdown')
...
@bot.message_handler(func=lambda message: message.text in
                                          [f'{white_square}{white_player}' for white_player in
                                           (current_game.white_team + ['own_black'])] +
                                          [f'{black_square}{black_player}' for black_player in
                                           (current_game.black_team + ['own_white'])])
def button(message):
    current_game.scorers.append(message.text[1:])
